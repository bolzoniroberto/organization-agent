...
import StepMode, { type ImportMode } from './import/StepMode'
...
const STEP_LABELS = ['Upload', 'Entità', 'Modalità', 'Mapping', 'Preview']

export default function ImportEnrichView() {
  ...
  const loadPreview = async () => {
    if (!file || !entity || !mode) return
    setDryRunLoading(true)
    try {
      const result = await api.import.dryRun({ file, entity, mode, mapping, sheetName, keyField: keyField || undefined })
  ...
  if (importDone) {
    return (
      <div className="flex flex-col items-center justify-center h-full gap-4">
        ...
        <h2 className="text-lg font-semibold text-slate-200">Arricchimento completato</h2>
        ...
        <button onClick={() => { setStep(1); setFile(null); setEntity(null); setMode(null); setMapping({}); setKeyField(''); setImportDone(null) }}
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="px-6 py-4 bg-slate-900 border-b border-slate-700">
        <h2 className="text-sm font-semibold text-slate-200 mb-3">Arricchimento Puntuale</h2>
          ...
          {STEP_LABELS.map((label, i) => {
            ...
            return (
              <div key={s} className="flex items-center gap-2">
                ...
                {i < 4 && <span className="text-slate-700 text-sm">›</span>}
              </div>
            )
          })}
        </div>
