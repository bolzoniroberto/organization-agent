...
export const api = {
  org: {
    list: (showDeleted = false, includeVars = false): Promise<NodoOrganigramma[]> =>
      fetch(`/api/org?showDeleted=${showDeleted}&includeVars=${includeVars}`).then(r => json(r)),

    tree: (): Promise<NodoOrganigramma[]> =>
      fetch('/api/org/tree').then(r => json(r)),

    get: (id: string): Promise<NodoOrganigramma | null> =>
      fetch(`/api/org/${encodeURIComponent(id)}`).then(r => r.status === 404 ? null : json(r)),
    ...
    update: (id: string, data: Partial<NodoOrganigramma>): Promise<{ success: boolean; error?: string }> =>
      fetch(`/api/org/${encodeURIComponent(id)}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(data)
      }).then(r => json(r)),
    ...
    delete: (id: string): Promise<{ success: boolean; message?: string }> =>
      fetch(`/api/org/${encodeURIComponent(id)}`, { method: 'DELETE' }).then(r => json(r)),

    restore: (id: string): Promise<{ success: boolean }> =>
      fetch(`/api/org/${encodeURIComponent(id)}/restore`, { method: 'POST' }).then(r => json(r)),
  },

  persone: {
    list: (showDeleted = false): Promise<Persona[]> =>
      fetch(`/api/persone?showDeleted=${showDeleted}`).then(r => json(r)),

    get: (cf: string): Promise<Persona | null> =>
      fetch(`/api/persone/${encodeURIComponent(cf)}`).then(r => r.status === 404 ? null : json(r)),
    ...
    update: (cf: string, data: Partial<Persona>): Promise<{ success: boolean; error?: string }> =>
      fetch(`/api/persone/${encodeURIComponent(cf)}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(data)
      }).then(r => json(r)),
    ...
    delete: (cf: string): Promise<{ success: boolean }> =>
      fetch(`/api/persone/${encodeURIComponent(cf)}`, { method: 'DELETE' }).then(r => json(r)),

    restore: (cf: string): Promise<{ success: boolean }> =>
      fetch(`/api/persone/${encodeURIComponent(cf)}/restore`, { method: 'POST' }).then(r => json(r)),
  },

  timesheet: {
    list: (): Promise<SupervisioneTimesheet[]> =>
      fetch('/api/timesheet').then(r => json(r)),

    get: (cf: string): Promise<SupervisioneTimesheet | null> =>
      fetch(`/api/timesheet/${encodeURIComponent(cf)}`).then(r => r.status === 404 ? null : json(r)),

    delete: (cf: string, hard = false): Promise<{ success: boolean; error?: string }> =>
      fetch(`/api/timesheet/${encodeURIComponent(cf)}${hard ? '?hard=true' : ''}`, { method: 'DELETE' }).then(r => json(r)),
  },

  tns: {
    list: (): Promise<RuoloTns[]> =>
      fetch('/api/tns').then(r => json(r)),

    get: (cf: string): Promise<RuoloTns | null> =>
      fetch(`/api/tns/${encodeURIComponent(cf)}`).then(r => r.status === 404 ? null : json(r)),

    update: (cf: string, data: Partial<RuoloTns>): Promise<{ success: boolean; error?: string }> =>
      fetch(`/api/tns/${encodeURIComponent(cf)}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(data)
      }).then(r => json(r)),
    ...
    delete: (cf: string, hard = false): Promise<{ success: boolean; error?: string }> =>
      fetch(`/api/tns/${encodeURIComponent(cf)}${hard ? '?hard=true' : ''}`, { method: 'DELETE' }).then(r => json(r)),
  },
  ...
  variabili: {
    ...
    updateDefinizione: (id: number, data: Partial<VariabileOrgDefinizione>): Promise<{ success: boolean; error?: string }> =>
      fetch(`/api/variabili/definizioni/${id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(data)
      }).then(r => json(r)),
  ...
  struttureTns: {
    list: (): Promise<StrutturaTns[]> =>
      fetch('/api/strutture-tns').then(r => json(r)),

    get: (codice: string): Promise<StrutturaTns | null> =>
      fetch(`/api/strutture-tns/${encodeURIComponent(codice)}`).then(r => r.status === 404 ? null : json(r)),

    create: (data: Partial<StrutturaTns>): Promise<{ success: boolean; error?: string }> =>
      fetch('/api/strutture-tns', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(data)
      }).then(r => json(r)),
    ...
    update: (codice: string, data: Partial<StrutturaTns>): Promise<{ success: boolean; error?: string }> =>
      fetch(`/api/strutture-tns/${encodeURIComponent(codice)}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(data)
      }).then(r => json(r)),
    ...
    delete: (codice: string): Promise<{ success: boolean; error?: string }> =>
      fetch(`/api/strutture-tns/${encodeURIComponent(codice)}`, { method: 'DELETE' }).then(r => json(r)),
  },
  ...
  import: {
    preview: async (file: File): Promise<{ sheetNames: string[]; headers: string[]; sampleRows: Record<string, unknown>[] }> => {
      ...
      fd.append('file', file)
      return fetch('/api/import/preview', { method: 'POST', body: fd }).then(r => json(r))
    },
    ...
    dryRun: async (options: {
      ...
      return fetch('/api/import/dry-run', { method: 'POST', body: fd }).then(r => json(r))
    },
    ...
    execute: async (options: {
      ...
      fd.append('sheetName', options.sheetName)
      if (options.keyField) fd.append('keyField', options.keyField)
      return fetch('/api/import/execute', { method: 'POST', body: fd }).then(r => json(r))
    },
  },
}
